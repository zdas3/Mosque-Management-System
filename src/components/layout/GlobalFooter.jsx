...
export default function GlobalFooter({ apiTimings }) {
    ...
    return (
        <footer className="bg-[#0a0a0a] text-gray-300 py-16 border-t border-white/5 relative overflow-hidden">
            ...
            <div className="absolute top-0 right-0 w-[400px] h-[400px] bg-[#065f46]/5 blur-[120px] rounded-full pointer-events-none" />
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12">
                    ...
                    <div className="space-y-6">
                        <div className="flex flex-col">
                            ...
                            <span className="text-xs font-bold tracking-widest text-[#f59e0b] uppercase mt-1">Islamic Cultural Center</span>
                        </div>
                    ...
                    <div>
                        ...
                        <ul className="space-y-4">
                            {['About Us', 'Administration', 'Rules & Regulations', 'Contact'].map((item) => (
                                <li key={item}>
                                    <Link href="#" className="text-sm text-gray-400 hover:text-[#0d9488] transition-colors duration-200 flex items-center gap-2 group">
                                        <span className="w-1.5 h-1.5 rounded-full bg-[#065f46] group-hover:bg-[#0d9488] transition-colors"></span>
                                        {item}
                                    </Link>
                                </li>
                            ))}
                    ...
                    <div>
                        ...
                        <ul className="space-y-5">
                            <li className="flex items-start gap-3 group">
                                <MapPin className="text-[#065f46] mt-0.5 flex-shrink-0 group-hover:text-[#0d9488] transition-colors" size={18} />
                                <span className="text-sm text-gray-400 font-light">123 Mosque Street, Community District, City 12345</span>
                            </li>
                            <li className="flex items-center gap-3 group">
                                <Phone className="text-[#065f46] flex-shrink-0 group-hover:text-[#0d9488] transition-colors" size={18} />
                                <span className="text-sm text-gray-400 font-light">+1 (234) 567-8900</span>
                            </li>
                            <li className="flex items-center gap-3 group">
                                <Mail className="text-[#065f46] flex-shrink-0 group-hover:text-[#0d9488] transition-colors" size={18} />
                        ...
                        <div className="bg-[#111] border border-white/5 rounded-2xl p-5 shadow-lg">
                            <ul className="space-y-3">
                                {['Fajr', 'Dhuhr', 'Asr', 'Maghrib', 'Isha'].map((prayer) => {
                                    ...
                                    if (apiTimings && apiTimings[prayer]) {
                                        const rawT = apiTimings[prayer].split(' ')[0];
                                        const [hr, min] = rawT.split(":").map(Number);
                                        let totalM = min + 3; // +3 minutes explicitly requested
                                        let newHr = hr + Math.floor(totalM / 60);
                                        let newMin = totalM % 60;
                                        const time24 = `${newHr.toString().padStart(2, '0')}:${newMin.toString().padStart(2, '0')}`;
                                        timeStr = formatTimeWithOffset(time24, 0);
                                    } else {
                                        // Fallback 
                                        if (prayer === 'Fajr') timeStr = "05:30 AM";
                                        else if (prayer === 'Dhuhr') timeStr = "12:45 PM";
                                        else if (prayer === 'Asr') timeStr = "04:15 PM";
                                        else if (prayer === 'Maghrib') timeStr = "06:10 PM";
                                        else if (prayer === 'Isha') timeStr = "07:45 PM";
                                    }
                                    ...
                                    return (
                                        <li key={prayer} className="flex justify-between items-center bg-white/5 px-3 py-2.5 rounded-xl border border-white/5 hover:border-white/10 transition-colors">
                                            ...
                                            <span className="text-sm text-[#0d9488] font-bold">{timeStr}</span>
                                        </li>
                                    );
                                })}
                            </ul>
